...
# Up Sample with Skip Connection
...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3,
        ...
        # DDAM
        self.ddam1 = DDAM(n_feat, bias=bias)
        self.ddam2 = DDAM(n_feat, bias=bias)
        self.ddam3 = DDAM(n_feat, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        x1ltop_fft = self.ddam1(x1ltop)
        x1rtop_fft = self.ddam1(x1rtop)
        x1lbot_fft = self.ddam1(x1lbot)
        x1rbot_fft = self.ddam1(x1rbot)

        feat1_ltop = self.stage1_encoder(x1ltop_fft)
        feat1_rtop = self.stage1_encoder(x1rtop_fft)
        feat1_lbot = self.stage1_encoder(x1lbot_fft)
        feat1_rbot = self.stage1_encoder(x1rbot_fft)
        ...
        x2top_fft = self.ddam2(x2top)
        x2bot_fft = self.ddam2(x2bot)

        x2top_cat = self.concat12(torch.cat([x2top_fft, x2top_samfeats], 1))
        x2bot_cat = self.concat12(torch.cat([x2bot_fft, x2bot_samfeats], 1))
        ...
        x3_fft = self.ddam3(x3)

        x3_cat = self.concat23(torch.cat([x3_fft, x3_samfeats], 1))
