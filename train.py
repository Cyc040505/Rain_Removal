...
def train():
    ...
    output_dir = os.path.join(opt.TRAINING.SAVE_DIR, 'output')
    model_dir = os.path.join(opt.TRAINING.SAVE_DIR, 'model')
    checkpoint_dir = os.path.join(opt.TRAINING.SAVE_DIR, 'checkpoint')
    ...
    # Loss
    criterion_char = Loss.CharbonnierLoss()
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
            ...
            if epoch % opt.TRAINING.SAVE_AFTER_EVERY == 0:
                torch.save({'epoch': epoch,
                            'state_dict': model_restoration.state_dict(),
                            'optimizer': optimizer.state_dict()
